# HAKO-NIWA（箱庭）— みんなが出会える場所（matching-app）

**HAKO-NIWA（箱庭）** は、安心できる場でゆっくり人と出会うためのサービス。男女3人ずつ・計6人で会う、新しいかたちの集まり。東京・恵比寿 / 池袋 / 銀座で、**LINE（LIFF）上**で動く（ホスティングは Vercel）。アプリ内チャットは無く、**会場は運営が手配して6人へLINEで通知**する設計。

> 「箱庭」＝小さくて手入れの行き届いた、安心できる庭。そこに人が集い、自然に出会う——というコンセプト。
...
**LINEログイン**: LIFFで起動すると「LINEではじめる」→ LINEログイン → `liff.getIDToken()` の id_token をサーバが LINE verify API で検証（iss/aud/exp/sub）してセッション発行（`src/app/_lib/liff-login.ts`・`src/lib/auth/line-verify.ts`）。LIFF未設定の開発環境では dev-login にフォールバック。実際にLINE上で動かすには、LINEコンソールでのLIFF endpoint URL登録と環境変数設定（殿の手動）が必要。
